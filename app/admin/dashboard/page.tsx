    ...
    <div style={{ minHeight: '100vh', background: '#F5F0E8', fontFamily: 'Inter, sans-serif' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 30, height: 30, background: '#085CF0', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900, color: 'white' }}>CS</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
            onClick={fetchData}
            style={{ background: 'none', border: '1px solid rgba(175,233,253,0.15)', borderRadius: 8, padding: '6px 12px', color: 'rgba(175,233,253,0.6)', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
          >
          ...
          <button
            ...
            disabled={loggingOut}
            style={{ background: 'rgba(224,49,49,0.1)', border: '1px solid rgba(224,49,49,0.2)', borderRadius: 8, padding: '6px 14px', color: '#E03131', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}
          >
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 14, marginBottom: 20 }}>
              ...
              <KpiCard
                ...
                value={data.stats.totalUnlocked}
                sub={`${data.stats.conversionRate}% taux de conv.`}
              ...
              <KpiCard
                ...
                color={scoreColor(data.stats.scoreAvg)}
              />
              <KpiCard
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 14, marginBottom: 24 }}>
              ...
              {/* Conversion */}
              <div style={{ background: 'white', borderRadius: 14, padding: '20px 24px', border: '1px solid rgba(9,38,118,0.08)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 12 }}>
                  <span style={{ fontSize: 32, fontWeight: 800, color: data.stats.conversionRate >= 30 ? '#2B9348' : data.stats.conversionRate >= 15 ? '#E8890C' : '#E03131', fontFamily: 'Syne, sans-serif' }}>
                ...
                <div style={{ height: 8, background: '#EDE8DF', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min(100, data.stats.conversionRate)}%`, background: data.stats.conversionRate >= 30 ? '#2B9348' : data.stats.conversionRate >= 15 ? '#E8890C' : '#E03131', borderRadius: 4, transition: 'width 1s ease' }} />
                </div>
              </div>
                ...
                {data.stats.topSectors.map((s, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: i < data.stats.topSectors.length - 1 ? 10 : 0 }}>
                      ...
                      <div style={{ flex: 1, height: 5, background: '#EDE8DF', borderRadius: 3 }}>
                        <div style={{ height: '100%', width: `${(s.count / max) * 100}%`, background: '#085CF0', borderRadius: 3, opacity: 0.6 + 0.4 * (s.count / max) }} />
                      </div>
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#050A34', width: 24, textAlign: 'right' as const }}>{s.count}</span>
                    </div>
                  )
                })}
              </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 0, flex: 1, minWidth: 200 }}>
            ...
            <button
              ...
              style={{ padding: '9px 16px', background: '#085CF0', color: 'white', border: 'none', borderRadius: '0 10px 10px 0', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}
            >
              Chercher
            </button>
          </div>
          ...
          {(search || filterSector || filterUnlocked) && (
            <button
              ...
              style={{ padding: '9px 14px', background: 'none', border: '1.5px solid #EDE8DF', borderRadius: 10, fontSize: 12, color: '#7A82A0', cursor: 'pointer' }}
            >
              ✕ Réinitialiser
            </button>
          )}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: 13 }}>
                ...
                <tbody>
                  {data.audits.map((audit, i) => (
                    <tr
                      key={audit.id}
                      style={{ borderBottom: i < data.audits.length - 1 ? '1px solid #F5F0E8' : 'none', transition: 'background 0.15s' }}
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px', whiteSpace: 'nowrap' as const }}>
                        ...
                        {/* Mini barre */}
                        <div style={{ width: 60, height: 3, background: '#EDE8DF', borderRadius: 2, marginTop: 4 }}>
                          <div style={{ height: '100%', width: `${audit.scoreGlobal}%`, background: scoreColor(audit.scoreGlobal), borderRadius: 2 }} />
                        </div>
                      </td>
                      ...
                      <td style={{ padding: '12px 16px', whiteSpace: 'nowrap' as const }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <a
                            ...
                            style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', background: 'rgba(8,92,240,0.08)', border: '1px solid rgba(8,92,240,0.15)', borderRadius: 7, fontSize: 11, fontWeight: 600, color: '#085CF0', textDecoration: 'none' }}
                          >
                          ...
                          {audit.isUnlocked && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', background: 'rgba(43,147,72,0.07)', border: '1px solid rgba(43,147,72,0.15)', borderRadius: 7, fontSize: 11, fontWeight: 600, color: '#2B9348', textDecoration: 'none' }}
                            >
...
function KpiCard({ label, value, sub, icon, color }: { label: string; value: string | number; sub?: string; icon: React.ReactNode; color: string }) {
  return (
    <div style={{ background: 'white', borderRadius: 14, padding: '20px 22px', border: '1px solid rgba(9,38,118,0.08)', display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ width: 32, height: 32, borderRadius: 9, background: `${color}12`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{icon}</div>
      </div>
      <div>
        <div style={{ fontSize: 28, fontWeight: 800, color: '#050A34', fontFamily: 'Syne, sans-serif', lineHeight: 1 }}>{value}</div>
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} style={{ display: 'flex', gap: 16, padding: '16px 20px', borderBottom: i < 5 ? '1px solid #F5F0E8' : 'none' }}>
          {[80, 160, 120, 60, 80, 140, 80].map((w, j) => (
            <div key={j} style={{ height: 12, width: w, background: '#F0EDE6', borderRadius: 6, animation: `pulse 1.5s ease-in-out ${j * 0.1}s infinite` }} />
          ))}
        </div>
      ))}
